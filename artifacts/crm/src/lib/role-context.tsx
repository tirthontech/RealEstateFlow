...
export type UserRole = "owner" | "cfo" | "manager" | "sales" | "employee";
...
export const ROLE_NAV_ACCESS: Record<UserRole, string[]> = {
  owner:    ["dashboard", "leads", "properties", "deals", "agents", "viewings", "whatsapp", "analytics", "commission", "integrations", "settings", "approvals"],
  cfo:      ["dashboard", "analytics", "commission", "settings"],
  manager:  ["dashboard", "leads", "properties", "deals", "agents", "viewings", "whatsapp", "analytics", "commission", "settings", "approvals"],
  sales:    ["dashboard", "leads", "properties", "viewings", "commission"],
...
export function RoleProvider({ children }: { children: React.ReactNode }) {
  ...
  const profile = ROLE_PROFILES.find(r => r.value === role) ?? ROLE_PROFILES[0];
  const can = (page: string) => ROLE_NAV_ACCESS[role].includes(page);

  return (
    <RoleContext.Provider value={{ role, setRole, profile, can }}>
      {children}
